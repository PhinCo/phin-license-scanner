( function(){
	...
	DirectoryScanRunner.prototype.logWarningDependencies = function( dependencies ){
		...
		_.each( dependencies, function( dependency ){
			if( dependency.licenses ){
				...
				if( _.intersection( licenses, warningLicenses ).length > 0 ){
					console.log("Warning: ".red + dependency.name.white + ", " + dependency.repo + " uses licenses: " + licenses.join(',') );
				}
			}
		})
	};
	...
	function outputAggregateDependencies( dependencies, options ){
		...
		var data = _.sortBy( _.uniqBy( clean, "name" ), "name" );
