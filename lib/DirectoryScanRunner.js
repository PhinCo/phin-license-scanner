( function(){
	...
	DirectoryScanRunner.run = function( runner, masterOptions ){
		var directories = _.map( runner.directories, function( options, path ){
			return { path: path, options: options };
		});

		Promise.mapSeries( directories, function( directoryNode ){
			var options = _.extend( masterOptions, directoryNode.options );
			return new DirectoryScanRunner( directoryNode.path, options ).perform();
		});
	};
