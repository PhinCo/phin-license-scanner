( function(){
	...
	class LicenseScanner {
		...
		static async scanDirectory( directory, options, licenseConfig ){
		...
		static async scanDirectories( directories, directoryOptions, licenseConfig ){
			const dependencies = {};

			for( let i = 0; i < directories.length ; i ++ ){
				const directory = directories[i];
				const options = directoryOptions[directory];
				dependencies[directory] = await LicenseScanner.scanDirectory( directory, options, licenseConfig );
			}
