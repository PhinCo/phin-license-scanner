( function(){
	...
	function PhinLicenseScanner( directory, licenseConfigFilePath ){
		licenseConfigFilePath = licenseConfigFilePath || LICENSE_CONFIG_FILE_PATH;
		this.directory = path.resolve( directory );
		this.isNodeProject = _isNodeProject( directory );
		this.isBowerProject = _isBowerProject( directory );
		this.licenseConfig = _loadLicenseConfigFile( licenseConfigFilePath );
		this.licenseConfig.filePath = licenseConfigFilePath;
	}
	...
	PhinLicenseScanner.prototype.scanForNodeDependencies = function(){
		...
		.then( function(){
			if( self.licenseConfig && self.licenseConfig.node ){
				_overrideWithKnownLicenses( output, self.licenseConfig.node );
				output = _filterExcludedDependencies( output, self.licenseConfig.exclude );
			}
		})
		.then( function(){
			return output;
	...
	PhinLicenseScanner.prototype.scanForBowerDependencies = function(){
		...
		.then( function( dependencies ){
			output = dependencies;
			if( self.licenseConfig && self.licenseConfig.bower ){
				_overrideWithKnownLicenses( output, self.licenseConfig.bower );
				output = _filterExcludedDependencies( output, self.licenseConfig.exclude );
			}
		})
		.then( function(){
	...
	PhinLicenseScanner.prototype.loadLicenseConfigFile = function( filepath ){
		this.licenseConfig = _loadLicenseConfigFile( filepath );
	};
	...
	function _filterExcludedDependencies( dependencies, exclusions ){
		return _.filter( dependencies, function( dependency ){
			var specificDependency = dependency.name;
			var globalDependency = dependency.name.split('@')[0];
			if( exclusions.indexOf( globalDependency ) !== -1 ) return false;
			if( exclusions.indexOf( specificDependency) !== -1 ) return false;
			return true;
		});
	}
