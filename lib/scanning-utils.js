( function(){
	...
	// TODO: Path and Depth are no longer computed correctly
	...
	exports.filterDependenciesWithUnknownLicenses = function( dependencies ){
		return _.filter( dependencies, dependency => dependency.licenses === "UNKNOWN" );
	};

	exports.filterDependeniesWithLicenseWarnings = function( dependencies, licenseConfig ){
		const warningLicensePatterns = _.get( licenseConfig, "warnOnLicenses" );
		if( _.size( warningLicensePatterns ) === 0 ) return [];

		return _.filter( dependencies, dependency => _checkLicensesForWarning( dependency.licenses, warningLicensePatterns ));
	};

	function _checkLicensesForWarning( licenses, warningLicensePatterns ){
		if( !_.isArray( licenses ) ) licenses = [licenses];

		let raiseWarning = false;

		_.each( licenses, license =>{
			_.each( warningLicensePatterns, ( pattern ) =>{
				if( _.isString( pattern && pattern === license ) ){
					raiseWarning = true;
					return false;
				}else if( _.isObject( pattern ) && pattern.pattern && new RegExp( pattern.pattern ).exec( license ) ){
					raiseWarning = true;
					return false;
				}
			});
